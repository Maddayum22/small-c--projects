...
namespace SudokuSolver.Strategies
{
    public class NakedPairStrategy : ISudokuStrategy
    {
        ...
        public NakedPairStrategy(SudokuMapper mapper)
        {
            _mapper = mapper;
        }
        ...
        private void EliminateNakedPairsFromOthersInRow(int[,] sudokuBoard, int givenRow, int givenCol)
        {
            ...
            for (int col = 0; col < sudokuBoard.GetLength(1); col++)
            {
               if (sudokuBoard[givenRow, col] != sudokuBoard[givenCol, givenCol] && sudokuBoard[givenRow, col].ToString().Length > 1)
                {
        ...
        private void EliminateNakedPair(int[,] sudokuBoard, int valuesToEliminate, int eliminateFromRow, int eliminateFromCol)
        {
            ...
            foreach (var value in valuesToEliminateArray)
            {
                sudokuBoard[eliminateFromRow, eliminateFromCol] = Convert.ToInt32(sudokuBoard[eliminateFromRow, eliminateFromCol]
                    .ToString().Replace(valuesToEliminate.ToString(), string.Empty));
            }
        }
